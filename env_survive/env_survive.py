...
class EnvSurvive(gym.Env):
    ...
    def __init__(self, path, seed=0, raw_vision=True, memory_task=False):
        ...
        if self.raw_vision:
            self.observation_space = spaces.Box(low=0.0, high=1.0,
                                                shape=(np.prod(self.vision_shape) + n_status,),
                                                dtype=np.float32)
        else:
            self.observation_space = spaces.Box(low=0.0, high=1.0, shape=(n_labels + n_status,), dtype=np.float32)
    ...
    def reset(self):
        ...
        if self.memory_task:
            obs = np.concatenate((self.cur_vision.flatten(),
                                  ...
                                  self.cur_status[0:1] / self.max_status), axis=0).astype(np.float32)  # 8,
        else:
            obs = np.concatenate((self.cur_vision.flatten(),
    ...
    def step(self, action, agent_info=None):
        if agent_info is not None and 'hidden' in agent_info:
            pre_hidden = agent_info['hidden']
            pre_vision = np.copy(self.cur_vision)
            pre_action = np.zeros((n_action,), np.int8)
            pre_action[action] = 1
            # print(pre_vision.shape, pre_hidden.shape, pre_action.shape, pre_hidden, agent_info)
        # Update status
        ...
        if self.cur_label == 1 and action == 0:
            if self.np_random.uniform(low=0.0, high=1.0, size=None) < 0.6:
                if self.memory_task:
                    return np.zeros((n_labels + n_status + 1 + n_labels + n_action),
                                    np.float32), self.death_rew, True, {'death_sign': 1}
                if not self.raw_vision:
        ...
        if p[0] < death_prob[self.cur_status[0]]:
            if self.memory_task:
                return np.zeros((n_labels + n_status + 1 + n_labels + n_action), np.float32), self.death_rew, True, {
                    'death_sign': 2}
            if not self.raw_vision:
        ...
        if self.memory_task:
            obs = np.concatenate((self.cur_vision.flatten(),
                                  self.cur_status / self.max_status,
                                  pre_hidden, pre_vision, pre_action), axis=0).astype(np.float32)  # 4+2+1+4+3,
        else:
            obs = np.concatenate((self.cur_vision.flatten(),
