...
class EnvSurvive(gym.Env):
    ...
    def __init__(self, dataset, vision_shape, seed=0, raw_vision=True, sequential_update=True, true_hidden=True):
        ...
        # give observation as fixed size or variable size
        ...
        if self.raw_vision:
            ...
            self.ob_dim = self.vision_index + n_status + pred_status + self.T * (self.vision_index + n_action)
            self.ob_dim_true = self.vision_size + n_status + pred_status + self.T * (self.vision_size + n_action)
    ...
    def reset(self):
        ...
        # print(self.cur_t, self.cur_vision, self.cur_label, self.cur_status / self.max_status)
        return obs
    ...
    def step(self, action, agent_info=None):
        ...
        if self.cur_label == 1 and action == 0:
            if self.np_random.uniform(low=0.0, high=1.0, size=None) < 0.6:
                if self.sequential_update:
                    return np.zeros(self.b2 + self.cur_t * self.i2, np.float32), self.death_rew, True, {'death_sign': 1}
                else:
        ...
        if p[0] < death_prob[self.cur_status[0]]:
            if self.sequential_update:
                return np.zeros(self.b2 + self.cur_t * self.i2, np.float32), self.death_rew, True, {'death_sign': 2}
            else:
        ...
        if p[1] < death_prob[self.cur_status[1]]:
            if self.sequential_update:
                return np.zeros(self.b2 + self.cur_t * self.i2, np.float32), self.death_rew, True, {'death_sign': 3}
            else:
        ...
        if self.sequential_update:
            pre_hidden = self.ep_status[0][0:pred_status]
            pre_va = np.concatenate(np.concatenate(self.ep_va))
            obs = np.concatenate((self.cur_vision.flatten(),
                                  self.cur_status / self.max_status,
                                  pre_hidden, pre_va), axis=0).astype(np.float32)  # 2+2+2+tx(2+3)
        else:
            if self.cur_t < self.T:
                pre_hidden = self.ep_status[0][0:pred_status]
                pre_va = np.concatenate(np.concatenate(self.ep_va))
                obs = np.concatenate((self.cur_vision.flatten(),
                                      self.cur_status / self.max_status,
        ...
        self.ep_status.append(self.cur_status / self.max_status)
        # print(self.cur_t, self.cur_vision, self.cur_label, self.cur_status / self.max_status, pre_hidden, pre_va)
    ...
    def obs_to_image_obs(self, obs):
        ...
        if obs_dim == self.a2:
            ...
            T = int((obs_dim - self.b2) / self.i2)
            new_obs_dim = self.b1 + T * self.i1
            img_obs = np.zeros(shape=(new_obs_dim,), dtype=np.float32)
            img_obs[:self.vision_size] = self.dataset[vision_map[int(obs[0])] + '_image'][int(obs[1])].flatten()
            img_obs[self.vision_size:self.a1] = obs[self.vision_index:self.a2]
            img_obs[self.a1:self.b1] = obs[self.a2:self.b2]

            for i in range(T):
                img_obs[self.b1 + i * self.i1:self.b1 + self.vision_size + i * self.i1] = \
                    self.dataset[vision_map[int(obs[self.b2 + i * self.i2])] + '_image'][int(obs[self.b2 + i * self.i2 + 1])].flatten()
                img_obs[self.b1 + i * self.i1 + self.vision_size:self.b1 + (i+1) * self.i1] = \
                    obs[self.b2 + i * self.i2 + self.vision_index:self.b2 + (i+1) * self.i2]
