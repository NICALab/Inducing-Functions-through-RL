...
from rlkit.torch.networks.custom import BaselineSeqCifar
...
def experiment(variant):
    ...
    qf = BaselineSeqCifar(raw_vision=raw_vision, pred_status=pred_status, memory_variation=memory_variation)
    target_qf = BaselineSeqCifar(raw_vision=raw_vision, pred_status=pred_status, memory_variation=memory_variation)
    ...
    if raw_vision:
        if sequential_update:
            if memory_variation == 0:
                exp_prefix = 'cifar10_sequence'
            elif memory_variation == 1:
                exp_prefix = 'cifar10_sequence_ext'
                if fixed_sequence_use_prediction:
                    exp_prefix = exp_prefix + '_use_pred'
            elif memory_variation == 2:
                exp_prefix = 'cifar10_sequence_sep'
            seed = fixed_sequence
        else:
            if memory_variation == 0:
                exp_prefix = 'cifar10_random'
            elif memory_variation == 1:
                exp_prefix = 'cifar10_random_ext'
            elif memory_variation == 2:
                exp_prefix = 'cifar10_random_sep'
            seed = T
    else:
