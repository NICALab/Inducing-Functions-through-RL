        ...
        if info_key in paths[0]:
            ...
            for k in all_env_infos[0].keys():
                if k == 'death_sign':
                    ...
                    death_p, death_h, death_s = np.sum(final_ks == 1), np.sum(final_ks == 2), np.sum(final_ks == 3)
                    n_ep = death_p + death_h + death_s
                    statistics['death_p'] = death_p / n_ep
                    statistics['death_h'] = death_h / n_ep
                    statistics['death_s'] = death_s / n_ep
                '''
                final_ks = np.array([info[k][-1] for info in all_env_infos])
